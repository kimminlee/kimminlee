...
* **Problem:** 반복되는 위젯과 카드 UI를 매번 새로 구현함에 따라 코드 중복이 발생하고, 스타일 변경 시 수정 범위가 넓어지는 비효율 발생.
* **Solution:** 공통 UI 요소를 식별하여 Props 기반의 재사용 가능한 컴포넌트*로 추상화하고, TypeScript Interface로 타입 안정성(Type Safety) 확보.
